...
class CoCom(Graal):
    ...
    def __init__(self, uri, git_path, worktreepath=DEFAULT_WORKTREE_PATH, exec_path=None,
        ...
        self.analyzer_factory = CoComAnalyzerFactory()
        self.composer = None
        self.file_analyzers = []
        self.repo_analyzers = []
        # self.analyzer = None
    ...
    def fetch(self, category=CATEGORY_COCOM_LIZARD_FILE, paths=None,
        ...
        if not self.analyzer_factory.exists(category):
            raise GraalError(cause="Unknown category %s" % category)

        self.composer = self.analyzer_factory.get_composer(CATEGORY_COCOM_LIZARD_FILE)
        ...
        if self.analyzer_kind in [LIZARD_FILE, SCC_FILE]:
            for committed_file in commit['files']:
                ...
                for file_analyzer in self.file_analyzers:
                    file_info = file_analyzer.analyze(local_path)
