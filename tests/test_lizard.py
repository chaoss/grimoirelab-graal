...
class TestLizard(TestCaseAnalyzer):
    ...
    def test_analyze_file_details(self):
        ...
        self.assertEqual(type(result['funs']), dict)

        for fd in result['funs']:
            self.assertIn('ccn', fd)
            self.assertEqual(type(fd['ccn']), int)
            self.assertIn('tokens', fd)
            self.assertEqual(type(fd['tokens']), int)
            self.assertIn('loc', fd)
            self.assertEqual(type(fd['loc']), int)
            self.assertIn('lines', fd)
            self.assertEqual(type(fd['lines']), int)
            self.assertIn('name', fd)
            self.assertEqual(type(fd['name']), str)
            self.assertIn('args', fd)
            self.assertEqual(type(fd['args']), int)
            self.assertIn('start', fd)
            self.assertEqual(type(fd['start']), int)
            self.assertIn('end', fd)
            self.assertEqual(type(fd['end']), int)
